...
title: "Closure"
date: "2021-04-22"
# coverUrl: "https://images.unsplash.com/photo-1605887075098-77bf2157632b?ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&ixlib=rb-1.2.1&auto=format&fit=crop&w=633&q=80"
...
<span class="photo-reference">Plato Terentev 님의 사진, 출처: Pexels
</span>
...
번역: Closure(이하 클로저)는 함수와 그 주변 상태(Lexical Environment)의 조합이다. 클로저가 있기 때문에 inner함수는 outer함수의 scope에 접근할 수 있다. 자바스크립트에서 클로저는 함수가 생성될 때마다 동시에 생성된다.
...
내가 이해한 바로는 위에 어려운 이야기와 관련이 있다. `선언 시점`이 중요한 키워드인데, inner함수가 선언될 당시에는 name변수가 있었고, 이것을 outerEnvironmentReference로 inner함수의 Lexical Environment가 형성됐다. 그러므로 outer함수는 종료됐지만, inner함수의 Lexical Environment는 자신이 태어날 때 name이라는 변수가 있었다는걸 기억하고 있다.(뭔가 멋진데??)
...
1. 동일한 outer 함수 안에서 만들어진 getName과 setName은 생성 당시의 stable함수의 Lexical Environment를 참조했으므로 지역변수 name을 `공유`한다.

1. 하지만 main과 sub는 name을 공유하지 않는다. 맨 위에서 말했다시피 클로저는 함수 생성시에 형성된다. getName과 setName은 stable함수가 실행될 때 생성되므로, main에서 한 번 sub에서 한 번 따로따로 생성되었다. 그러므로 `둘의 클로저는 다르며, 독립적`이다.

1. stable의 지역변수 name은 오로지 getName과 setName에서만 접근이 가능하다. JavaScript는 기본적으로 private한 속성을 지원하지 않는데, 클로저의 이러한 특성을 이용해서 `private 변수`로 활용하도록 응용할 수 있다. (비밀변수, private변수라는 단어는 MDN에서는 찾을 수 없었다. 아마도 편의상 그렇게 부르는 것 같다)
...
for (var i = 0; i < 5; i++) {
  arr[i] = ((x: number) => () => x)(i) // arr[i] = () => i 에서 변경
}
...
아마도 여기서 클로저라는 명칭이 비롯된 것 같다. 전역환경으로 나가는 문을 닫고 내부에 그와 구별되는 Lexical Environment를 만들어 놓는다는 의미가 아닐까 생각해 본다.
